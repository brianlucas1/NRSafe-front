...
export class PermissoesComponent implements OnInit {
  ...
  salvarTudo() {
    ...
    const proximo = () => {
      const p = seq.shift();
      if (!p) { this.loading = false; this.msg.add({severity:'success', summary:'Sucesso', detail:'Permissões atualizadas'}); return; }
  ...
  private carregarPapeis() {
    ...
    this.clienteService.listarPapeis().subscribe({
      next: (dados) => { this.papeis = dados ?? []; this.loading = false; },
      error: () => { this.papeis = []; this.loading = false; this.msg.add({severity:'error', summary:'Erro', detail:'Falha ao carregar permissões'}); }
    });
  }
}
